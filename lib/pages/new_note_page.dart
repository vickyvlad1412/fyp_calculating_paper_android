...
class NewNotePage extends StatefulWidget {
  ...
  const NewNotePage({super.key, required this.note});
...
class NewNotePageState extends State<NewNotePage> {
  ...
  void initState() {
    ...
    //Initialize the text controllers with the note data
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      // Only auto-focus if this is a new note (no saved blocks)
      if (widget.note.serializedBlocks.trim().isEmpty) {
        Future.delayed(const Duration(milliseconds: 100), () {
          _contentFocusNode.requestFocus(); // Show default keyboard initially
        });

        Future.delayed(const Duration(milliseconds: 200), () {
          setState(() {
            _isCustomKeyboardVisible = true;
            SystemChannels.textInput.invokeMethod('TextInput.hide'); // Hide default keyboard
          });
        });
      }
    });
  ...
  void saveNote() {
    final header = _titleController.text;

    if (header.isNotEmpty && blocks.isNotEmpty) {
      //Serialize blocks
      widget.note.header = _titleController.text;
      widget.note.blocks = blocks;

      // Add note to database
      context.read<NoteDatabase>().addNote(
        widget.note.header,
        widget.note.serializedBlocks,
      );
      ...
      showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text(
              'Both header and text must be filled!',
              style: TextStyle(
                color: Theme.of(context).colorScheme.inversePrimary,
              ),
            ),
